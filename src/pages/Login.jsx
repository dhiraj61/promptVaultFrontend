...
const Login = () => {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-gray-50 w-full min-h-dvh flex flex-col justify-center items-center gap-4 p-4">
      ...
      <input className="w-full h-16 border-b outline-0 p-4 " type="password" value={password} onChange={(e) => { setPassword(e.target.value) }} placeholder="******" />
      <button className="w-1/2 p-4 rounded-2xl shadow-2xl text-2xl active:scale-95">Login</button>
      <div className="flex flex-row gap-2"><h1 className="text-gray-400">Don't Have an Account ? </h1><h1 className="text-blue-500" onClick={()=>{navigate('/register')}}>Register</h1></div>
    </form>
  )
}
